...
class Thicc_Net(nn.Module):
    def __init__(self, layers,dila=1, dilb=2, dilc=3):
        ...
        self.bottom_layer = self.make_layer(self.chn*4,self.chn*8, layers[3], unet_block_large)
        ...
        self.upconv1 = unet_block(self.chn*8, self.chn*4)
        ...
        self.upconv2 = unet_block(self.chn*4, self.chn*2)
        ...
        self.upconv3 = unet_block(self.chn*2, self.chn)
    ...
    def forward(self, x):
        ...
        bottom = self.bottom_layer(pool_3) 

        up1 = self.up1_1x1(bottom)    
        cat1 = torch.cat([skip3,up1],dim=1) 
        out1 = self.upconv1(cat1)
        up2 = self.up2_1x1(out1)    
        cat2 = torch.cat([skip2,up2],dim=1) 
        out2 = self.upconv2(cat2)
        up3 = self.up3_1x1(out2)    
        cat3 = torch.cat([skip1,up3],dim=1) 
